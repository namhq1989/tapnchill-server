...
func (s server) registerGoalRoutes() {
	...
	g.DELETE("/goal/:id", func(c echo.Context) error {
		var (
			ctx         = c.Get("ctx").(*appcontext.AppContext)
			req         = c.Get("req").(dto.DeleteGoalRequest)
			performerID = ctx.GetUserID()
			goalID      = c.Param("id")
		)

		resp, err := s.app.DeleteGoal(ctx, performerID, goalID, req)
		if err != nil {
			return httprespond.R400(c, err, nil)
		}
		...
		return validation.ValidateHTTPPayload[dto.DeleteGoalRequest](next)
	})
}
