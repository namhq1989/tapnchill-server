...
func (s server) registerUserRoutes() {
	...
	g.GET("/payment-customer-portal-url", func(c echo.Context) error {
		var (
			ctx         = c.Get("ctx").(*appcontext.AppContext)
			req         = c.Get("req").(dto.GetPaymentCustomerPortalURLRequest)
			performerID = ctx.GetUserID()
		)

		resp, err := s.app.GetPaymentCustomerPortalURL(ctx, performerID, req)
		if err != nil {
			return httprespond.R400(c, err, nil)
		}
		...
		return validation.ValidateHTTPPayload[dto.GetPaymentCustomerPortalURLRequest](next)
	})
}
