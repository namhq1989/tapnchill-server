...
func (s server) registerUserRoutes() {
	...
	g.POST("/sign-in/anonymous", func(c echo.Context) error {
		var (
			ctx = c.Get("ctx").(*appcontext.AppContext)
			req = c.Get("req").(dto.AnonymousSignInRequest)
		)

		resp, err := s.app.AnonymousSignIn(ctx, req)
		if err != nil {
		...
		return validation.ValidateHTTPPayload[dto.AnonymousSignInRequest](next)
	})
}
